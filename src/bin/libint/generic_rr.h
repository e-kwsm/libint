...
class GenericRecurrenceRelation : public RecurrenceRelation {
 public:
  ...
  static SafePtr<RRImpl> Instance(const SafePtr<TargetType>& Tint,
                                  unsigned int dir) {
    // screen out calls with nondefault extra parameters
    if (!RRImpl::directional() && dir != 0) return SafePtr<RRImpl>();
...
class ChildFactory {
 public:
  ...
  /// make_child
  const SafePtr<DGVertex>& make_child(const F& A, const F& B, const F& C,
                                      const F& D,
                                      const AuxIndexType& aux = AuxIndexType(),
                                      const OperType& oper = OperType()) {
    auto i = static_pointer_cast<DGVertex, ChildType>(
        ChildType::Instance(A, B, C, D, aux, oper));
    return rr_->add_child(i);
  }
  /// make_child
  const SafePtr<DGVertex>& make_child(const F& A, const F& B,
                                      const AuxIndexType& aux = AuxIndexType(),
                                      const OperType& oper = OperType()) {
    auto i = static_pointer_cast<DGVertex, ChildType>(
  ...
  const SafePtr<DGVertex>& make_child(
  ...
  void wedge(
      const LinearCombination<SafePtr<DGVertex>, BraketPair<F, PBra> >& bra_lc,
      const BraketPair<F, PKet>& ket, const AuxIndexType& aux = AuxIndexType(),
      const OperType& oper = OperType()) {
    using namespace libint2::prefactor;
