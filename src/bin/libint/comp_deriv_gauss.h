...
class CR_DerivGauss
 ...
 private:
  ...
  bool has_generic(
      const SafePtr<CompilationParameters>& cparams) const override;
  ...
  std::string generic_instance(const SafePtr<CodeContext>& context,
...
CR_DerivGauss<IntType, part, where, trans_inv_part,
              trans_inv_where>::CR_DerivGauss(const SafePtr<TargetType>& Tint,
                                              unsigned int dir)
    : ParentType(Tint, dir) {
...
bool CR_DerivGauss<IntType, part, where, trans_inv_part, trans_inv_where>::
    has_generic(const SafePtr<CompilationParameters>& cparams) const {
...
template <class IntType, int part, FunctionPosition where, int trans_inv_part,
          FunctionPosition trans_inv_where>
std::string CR_DerivGauss<
    IntType, part, where, trans_inv_part,
    trans_inv_where>::generic_instance(const SafePtr<CodeContext>& context,
