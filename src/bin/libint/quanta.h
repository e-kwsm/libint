...
QuantumNumbers<T, N>::QuantumNumbers(const SafePtr<QuantumSet>& sptr) {
  const SafePtr<QuantumNumbers<T, N> > sptr_cast =
      dynamic_pointer_cast<QuantumNumbers, QuantumSet>(sptr);
#if CHECK_SAFETY
  if (sptr_cast == 0)
    throw std::runtime_error(
        "QuantumNumbers<T,N>::QuantumNumbers(const SafePtr<QuantumSet>& sptr) "
        "-- type of sptr is incompatible with QuantumNumbers");
#endif
...
QuantumNumbers<T, N>::QuantumNumbers(
  ...
  if (sptr_cast == 0)
    throw std::runtime_error(
        ...
        "SafePtr<ConstructablePolymorphically>& sptr) -- type of sptr is "
        "incompatible with QuantumNumbers");
#endif
...
QuantumNumbersA<T, N>::QuantumNumbersA(const SafePtr<QuantumSet>& sptr) {
  const SafePtr<QuantumNumbersA<T, N> > sptr_cast =
      dynamic_pointer_cast<QuantumNumbersA, QuantumSet>(sptr);
#if CHECK_SAFETY
  if (sptr_cast == 0)
    throw std::runtime_error(
        "QuantumNumbersA<T,N>::QuantumNumbersA(const SafePtr<QuantumSet>& "
        "sptr) -- type of sptr is incompatible with QuantumNumbersA");
#endif
...
QuantumNumbersA<T, N>::QuantumNumbersA(
  ...
  if (sptr_cast == 0)
    throw std::runtime_error(
        ...
        "SafePtr<ConstructablePolymorphically>& sptr) -- type of sptr is "
        "incompatible with QuantumNumbersA");
#endif
  T* qn = sptr_cast->qn_;
  for (int i = 0; i < N; i++) qn_[i] = qn[i];
}
