...
void GenerateCode(const SafePtr<DirectedGraph>& dg,
                  const SafePtr<CodeContext>& context,
                  const SafePtr<CompilationParameters>& cparams,
                  const SafePtr<Strategy>& strat, const SafePtr<Tactic>& tactic,
...
void BuildTest(const std::vector<SafePtr<Integral> >& targets,
 ...
 */
template <class Integral, bool GenAllCode>
void __BuildTest(const std::vector<SafePtr<Integral> >& targets,
                 const SafePtr<CompilationParameters>& cparams,
                 unsigned int size_to_unroll, std::ostream& os = std::cout,
                 const SafePtr<Tactic>& tactic = SafePtr<Tactic>(
...
void BuildTest(const std::vector<SafePtr<Integral> >& targets,
  ...
  SafePtr<Tactic> tactic;
  {
    typedef GenIntegralSet_11_11<typename Integral::BasisFunctionType,
        ...
        genint_11_11_t;
    SafePtr<genint_11_11_t> cast_ptr =
    ...
    if (cast_ptr) {
      ...
      tactic =
          SafePtr<Tactic>(new FirstChoiceTactic<StdRandomizePolicy>(rpolicy));
    }
  }
