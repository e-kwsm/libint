...
class HRR : public RecurrenceRelation {
 public:
  ...
  /** Use Instance() to obtain an instance of RR. This function is provided to
   avoid issues with getting a SafePtr from constructor (as needed for registry
   to work).
 ...
 private:
  ...
  SafePtr<ImplicitDimensions> adapt_dims_(
      const SafePtr<ImplicitDimensions>& dims) const override;
...
bool HRR<IntType, F, part, loc_a, pos_a, loc_b, pos_b>::register_with_rrstack()
  ...
  if (!nonzero_quanta) {
    ...
    SafePtr<ThisType> this_ptr = const_pointer_cast<ThisType, const ThisType>(
  ...
  SafePtr<DummyIntegral> dummy_integral =
...
HRR<IntType, F, part, loc_a, pos_a, loc_b, pos_b>::adapt_dims_(
  ...
  SafePtr<ImplicitDimensions> localdims(
