...
void DGVertex::add_exit_arc(const SafePtr<DGArc>& arc) {
  if (can_add_arcs_) {
...
void DGVertex::del_exit_arc(const SafePtr<DGArc>& arc) {
  if (can_add_arcs_) {
    if (!children_.empty()) {
...
void DGVertex::replace_exit_arc(const SafePtr<DGArc>& A,
...
void DGVertex::add_entry_arc(const SafePtr<DGArc>& arc) {
...
void DGVertex::del_entry_arc(const SafePtr<DGArc>& arc) {
  if (!parents_.empty()) {
...
const SafePtr<DGArc>& DGVertex::exit_arc(const SafePtr<DGVertex>& v) const {
...
void DGVertex::refer_this_to(const SafePtr<DGVertex>& V) {
...
bool UnrolledIntegralSet::operator()(const SafePtr<DGVertex>& V) {
...
bool NotUnrolledIntegralSet::operator()(const SafePtr<DGVertex>& V) {
  return !UnrolledIntegralSet()(V);
}

bool IntegralInTargetIntegralSet::operator()(const SafePtr<DGVertex>& V) {
