...
void test0() {
  SafePtr<TwoPRep_11_11_sq> pppp_quartet =
      TwoPRep_11_11_sq::Instance(sh_p, sh_p, sh_p, sh_p, 0u);
  SafePtr<DGVertex> pppp_ptr =
...
void test1() {
  ...
  for (unsigned int m = 0; m < MemoryManagerFactory::ntypes; m++) {
    ...
    dg_xxxx3->generate_code(context, memman, ImplicitDimensions::default_dims(),
                            SafePtr<CodeSymbols>(new CodeSymbols),
...
void RunBuildTest(const typename Integral::BasisFunctionType& f1,
  ...
  RunTest(
      boost::bind(__BuildTest<Integral, false>, targets, cparams,
                  size_to_unroll, boost::ref(cout),
                  SafePtr<Tactic>(new FirstChoiceTactic<DummyRandomizePolicy>),
...
void RunBuildTest(const typename Integral::BasisFunctionType& f1,
  ...
  RunTest(boost::bind(
              ...
              boost::ref(cout),
              SafePtr<Tactic>(new FirstChoiceTactic<DummyRandomizePolicy>),
              SafePtr<MemoryManager>(new WorstFitMemoryManager), descr_label),
          descr_label);
}
...
void test7() {
  ...
  SafePtr<TwoPRep_11_11_sq> target =
