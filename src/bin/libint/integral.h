...
class IntegralSet {
 public:
  ...
  /// Obtain pointers to ith BasisFunctionSet for particle p in bra
  virtual const SafePtr<BasisFunctionSet> bra(unsigned int p,
                                              unsigned int i) const = 0;
  /// Obtain pointers to ith BasisFunctionSet for particle p in ket
  virtual const SafePtr<BasisFunctionSet> ket(unsigned int p,
                                              unsigned int i) const = 0;
#endif
};
...
class GenIntegralSet
  ...
  */
  ...
  static const SafePtr<GenIntegralSet> Instance(const BraSetType& bra,
...
void GenIntegralSet<Op, BFS, BraSetType, KetSetType, AuxQuanta>::unregister()
  ...
  singl_manager_.remove(const_pointer_cast<this_type, const this_type>(
      EnableSafePtrFromThis<this_type>::SafePtr_from_this()));
}
...
#else
  // compute size
  SafePtr<this_type> this_ptr = const_pointer_cast<this_type, const this_type>(
      EnableSafePtrFromThis<GenIntegralSet>::SafePtr_from_this());
  SafePtr<SubIteratorBase<this_type> > siter(
