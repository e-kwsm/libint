...
class apply_strategy {
 public:
  struct Impl {
    Impl(const SafePtr<DirectedGraph>& dg, const SafePtr<IntType>& integral,
         const SafePtr<Tactic>& tactic)
...
//        }
      ...
      if (can_unroll) {
        ...
        SafePtr<apply_strategy_t_impl> applier_impl(
...
class match_first_inttype {
 public:
  struct Impl {
    Impl(const SafePtr<DirectedGraph>& dg, const SafePtr<DGVertex>& integral,
         const SafePtr<Tactic>& tactic)
        : dg_(dg),
          integral_(integral),
          tactic_(tactic),
...
SafePtr<RecurrenceRelation> Strategy::optimal_rr(
    const SafePtr<DirectedGraph>& graph, const SafePtr<DGVertex>& integral,
    const SafePtr<Tactic>& tactic) {
#if 0
  {
  ...
  SafePtr<match_first_inttype::Impl> matcher_impl(
