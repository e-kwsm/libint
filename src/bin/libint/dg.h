...
class DirectedGraph : public EnableSafePtrFromThis<DirectedGraph> {
 ...
 private:
   ...
   */
  template <class I, class RR>
  void recurse(const SafePtr<I>& vertex) {
    ...
    for (int c = 0; c < num_children; c++) {
      ...
      if (child_cast == 0)
        throw std::runtime_error(
            "DirectedGraph::recurse(const SafePtr<I>& vertex) -- dynamic cast "
            "failed, most probably this is a logic error!");
   ...
   */
  template <class RR>
  void recurse(const SafePtr<DGVertex>& vertex) {
    SafePtr<DGVertex> dgvertex = add_vertex(vertex);
    if (dgvertex != vertex) return;
  ...
  /// graph
  SafePtr<DGVertex> insert_expr_at(
   ...
   */
  bool remove_vertex_at(const SafePtr<DGVertex>& v1,
  ...
  void assign_oper_symbol(const SafePtr<CodeContext>& context,
                          SafePtr<DGVertex>& v);
