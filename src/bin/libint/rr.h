...
class RecurrenceRelation : public EnableSafePtrFromThis<RecurrenceRelation> {
 ...
 protected:
  ...
  bool register_with_rrstack() {
    ...
    SafePtr<RRStackBase<RecurrenceRelation> > rrstack =
        RRStackBase<RecurrenceRelation>::Instance();
    SafePtr<RR> this_ptr = const_pointer_cast<RR, const RR>(
 ...
 private:
  ...
  virtual SafePtr<ImplicitDimensions> adapt_dims_(
      const SafePtr<ImplicitDimensions>& dims) const;
  ...
  /// code
  virtual std::string generic_instance(const SafePtr<CodeContext>& context,
...
//                                                const SafePtr<DGVertex>& B);
template <typename T>
SafePtr<RecurrenceRelation::ExprType> operator*(
    const SafePtr<RTimeEntity<T> >& A, const SafePtr<DGVertex>& B);
template <typename T>
SafePtr<RecurrenceRelation::ExprType> operator*(
    const SafePtr<CTimeEntity<T> >& A, const SafePtr<DGVertex>& B);
};  // namespace algebra
