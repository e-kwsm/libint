...
class MemoryBlock {
 public:
  ...
  static bool is_free(const SafePtr<MemoryBlock>& i) { return i->free(); }
...
class MemoryManager {
 ...
 private:
  ...
  SafePtr<MemBlock> merge_blocks(const SafePtr<MemBlock>& left,
 ...
 protected:
  ...
  SafePtr<MemBlock> steal_from_block(const SafePtr<MemBlock>& blk,
                                     const Size& size);
