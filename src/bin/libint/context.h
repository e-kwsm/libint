...
class CodeContext {
  ...
  */
   ...
   */
  virtual SafePtr<ForLoop> for_loop(
...
class CppCodeContext : public CodeContext,
                       public EnableSafePtrFromThis<CppCodeContext> {
 public:
  CppCodeContext(const SafePtr<CompilationParameters>& cparams,
  ...
  SafePtr<ForLoop> for_loop(std::string& varname,
